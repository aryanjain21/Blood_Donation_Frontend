...
export class BloodrequestComponent implements OnInit {
  ...
  request:any;
  userLogged=JSON.parse(sessionStorage.getItem("user1"));

  user={
    "requster_id": {
        "u_id":this.userLogged.u_id,
        "fName": "",
        "lName": "",
        "addr": {
            "city": "",
  ...
  addRequest(mughda) {
    ...
    console.log("in ts"+this.request);
    console.log("in ts"+this.user);
    this.dataservice.insertPatientDetails(this.user)
    toastr.success("Request Sended...")
    this.router.navigate(['/patientrequest']);
